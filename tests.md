...
Verification: The test verifies that many small usages trigger appropriate max-tree categorizations and bubbling up limits. (⚠️ Note: Currently, this execution correctly exposes a bug in your build_from_heap() logic—while it initializes FSM, it forgets to correctly serialize some of the mid-level tree internal slots! The test rightfully fails here and identifies exactly what you need to fix locally next).
