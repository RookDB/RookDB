...
impl BufferManager {
    ...
    pub fn load_csv_into_pages(
        ...
        // Reset buffer so stale pages from a previous load don't carry over
        self.pages.clear();
        let mut fresh_header = Page::new();
        init_page(&mut fresh_header);
        self.pages.push(fresh_header);
        ...
        // Ensure first data page exists, then start writing there (index 1, not 0 which is header)
        if self.pages.len() == 1 {
            self.allocate_page();
        }
        ...
        for (i, line) in lines.enumerate() {
            ...
            loop {
                ...
                page.data[item_id_pos + 4..item_id_pos + 6]
                    .copy_from_slice(&(tuple_len as u16).to_le_bytes()); // length: u16
                page.data[item_id_pos + 6..item_id_pos + 8]
