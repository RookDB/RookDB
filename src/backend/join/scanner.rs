//! Sequential tuple scanner for reading all rows from a table file.
...
/// Page-at-a-time sequential scanner over a table's heap file.
...
impl TupleScanner {
    /// Open a table by name and prepare for sequential scan.
    pub fn new(db: &str, table: &str, catalog: &Catalog) -> io::Result<TupleScanner> {
        ...
        Ok(TupleScanner { file, schema, current_page: 1, current_slot: 0, total_pages })
    }

    /// Create a scanner from an already-open file (used for temp files).
    pub fn from_file(mut file: File, schema: Vec<Column>) -> io::Result<TupleScanner> {
        let total_pages = page_count(&mut file)?;
        Ok(TupleScanner { file, schema, current_page: 1, current_slot: 0, total_pages })
    }

    /// Return the next tuple, advancing across pages as needed.
    pub fn next_tuple(&mut self) -> Option<Tuple> {
        loop {
            if self.current_page >= self.total_pages { return None; }
            let mut page = Page::new();
            if read_page(&mut self.file, &mut page, self.current_page).is_err() { return None; }
    ...
    /// Read all remaining tuples into a Vec.
    pub fn collect_all(&mut self) -> Vec<Tuple> {
        let mut tuples = Vec::new();
        while let Some(t) = self.next_tuple() { tuples.push(t); }
        tuples
    }
}
