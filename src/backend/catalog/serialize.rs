...
fn read_u8(c: &mut Cursor<&[u8]>) -> io::Result<u8> {
    let mut b = [0u8; 1]; c.read_exact(&mut b)?; Ok(b[0])
}
fn read_u16(c: &mut Cursor<&[u8]>) -> io::Result<u16> {
    let mut b = [0u8; 2]; c.read_exact(&mut b)?; Ok(u16::from_le_bytes(b))
}
fn read_u32(c: &mut Cursor<&[u8]>) -> io::Result<u32> {
    let mut b = [0u8; 4]; c.read_exact(&mut b)?; Ok(u32::from_le_bytes(b))
}
fn read_u64(c: &mut Cursor<&[u8]>) -> io::Result<u64> {
    let mut b = [0u8; 8]; c.read_exact(&mut b)?; Ok(u64::from_le_bytes(b))
}
fn read_i16(c: &mut Cursor<&[u8]>) -> io::Result<i16> {
    let mut b = [0u8; 2]; c.read_exact(&mut b)?; Ok(i16::from_le_bytes(b))
}
...
fn read_u32_arr(c: &mut Cursor<&[u8]>) -> io::Result<Vec<u32>> {
    ...
    for _ in 0..count { out.push(read_u32(c)?); }
    Ok(out)
}
...
pub fn serialize_column_tuple(
    ...
    match type_modifier {
        ...
        Some(TypeModifier::VarcharLen(n)) => { write_u8(&mut buf, 1); write_u16(&mut buf, *n); }
        Some(TypeModifier::Precision { precision, scale }) => {
            write_u8(&mut buf, 2); write_u8(&mut buf, *precision); write_u8(&mut buf, *scale);
        }
    }
...
pub fn deserialize_column_tuple(bytes: &[u8]) -> io::Result<(
    u32, u32, String, u16, DataType, Option<TypeModifier>, bool, Option<DefaultValue>, Vec<u32>
)> {
    let mut c = Cursor::new(bytes);
    ...
    let dt = DataType { type_oid, type_name, type_category, type_length, type_align, is_builtin: true };
    // TypeModifier
    let tm_flag = read_u8(&mut c)?;
    let type_modifier = match tm_flag {
        1 => Some(TypeModifier::VarcharLen(read_u16(&mut c)?)),
        2 => { let p = read_u8(&mut c)?; let s = read_u8(&mut c)?; Some(TypeModifier::Precision { precision: p, scale: s }) }
        _ => None,
    };
    let is_nullable = read_u8(&mut c)? != 0;
    // DefaultValue
    let has_default  = read_u8(&mut c)? != 0;
    let default_value = if has_default { Some(deserialize_default_value(&mut c)?) } else { None };
    let constraint_oids = read_u32_arr(&mut c)?;
    Ok((column_oid, table_oid, column_name, column_pos, dt, type_modifier, is_nullable, default_value, constraint_oids))
}
...
pub fn serialize_constraint_tuple(c: &Constraint) -> Vec<u8> {
    ...
    match &c.metadata {
        ...
        ConstraintMetadata::ForeignKey { referenced_table_oid, referenced_column_oids, on_delete, on_update } => {
...
pub fn deserialize_constraint_tuple(bytes: &[u8]) -> io::Result<Constraint> {
    ...
    let constraint_type = ConstraintType::from_u8(ct_byte)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, format!("unknown constraint type {}", ct_byte)))?;
    ...
    let metadata = match constraint_type {
        ...
        ConstraintType::Check    => {
            ...
            ConstraintMetadata::Check { check_expression: expr }
        }
    };
    Ok(Constraint { constraint_oid, constraint_name, constraint_type, table_oid, column_oids, metadata, is_deferrable })
}
...
pub fn deserialize_index_tuple(bytes: &[u8]) -> io::Result<Index> {
    ...
    Ok(Index { index_oid, index_name, table_oid, index_type, column_oids, is_unique, is_primary, index_pages })
}
...
pub fn deserialize_type_tuple(bytes: &[u8]) -> io::Result<DataType> {
    ...
    Ok(DataType { type_oid, type_name, type_category, type_length, type_align, is_builtin })
}
...
fn serialize_default_value(buf: &mut Vec<u8>, dv: &DefaultValue) {
    match dv {
        DefaultValue::Integer(v)         => { write_u8(buf, 1); buf.extend_from_slice(&v.to_le_bytes()); }
        DefaultValue::BigInt(v)          => { write_u8(buf, 2); buf.extend_from_slice(&v.to_le_bytes()); }
        DefaultValue::Float(v)           => { write_u8(buf, 3); buf.extend_from_slice(&v.to_le_bytes()); }
        DefaultValue::Double(v)          => { write_u8(buf, 4); buf.extend_from_slice(&v.to_le_bytes()); }
        DefaultValue::Str(s)             => { write_u8(buf, 5); write_str(buf, s); }
        DefaultValue::Boolean(b)         => { write_u8(buf, 6); write_u8(buf, *b as u8); }
        DefaultValue::Null               => { write_u8(buf, 7); }
        DefaultValue::CurrentTimestamp   => { write_u8(buf, 8); }
    }
}
...
fn deserialize_default_value(c: &mut Cursor<&[u8]>) -> io::Result<DefaultValue> {
    let tag = read_u8(c)?;
    match tag {
        1 => { let mut b = [0u8; 4]; c.read_exact(&mut b)?; Ok(DefaultValue::Integer(i32::from_le_bytes(b))) }
        2 => { let mut b = [0u8; 8]; c.read_exact(&mut b)?; Ok(DefaultValue::BigInt(i64::from_le_bytes(b))) }
        3 => { let mut b = [0u8; 4]; c.read_exact(&mut b)?; Ok(DefaultValue::Float(f32::from_le_bytes(b))) }
        4 => { let mut b = [0u8; 8]; c.read_exact(&mut b)?; Ok(DefaultValue::Double(f64::from_le_bytes(b))) }
        ...
        v => Err(io::Error::new(io::ErrorKind::InvalidData, format!("unknown default tag {}", v)))
    }
}
...
pub fn type_category_to_u8(tc: &TypeCategory) -> u8 {
    match tc { TypeCategory::Numeric => 1, TypeCategory::String => 2, TypeCategory::DateTime => 3, TypeCategory::Boolean => 4, TypeCategory::Binary => 5 }
}

pub fn type_category_from_u8(v: u8) -> TypeCategory {
    match v { 1 => TypeCategory::Numeric, 2 => TypeCategory::String, 3 => TypeCategory::DateTime, 4 => TypeCategory::Boolean, _ => TypeCategory::Binary }
}
