...
use crate::catalog::types::{
    Constraint, ConstraintMetadata, ConstraintType, CatalogDataType, DefaultValue, Index, IndexType,
...
pub fn serialize_type_tuple(dt: &CatalogDataType) -> Vec<u8> {
    let mut buf = Vec::new();
    write_u32(&mut buf, dt.type_oid);
    write_str(&mut buf, &dt.type_name);
    write_u8(&mut buf, type_category_to_u8(&dt.type_category));
...
pub fn deserialize_type_tuple(bytes: &[u8]) -> io::Result<CatalogDataType> {
    let mut c = Cursor::new(bytes);
    let type_oid = read_u32(&mut c)?;
    let type_name = read_str(&mut c)?;
    let type_category = type_category_from_u8(read_u8(&mut c)?);
