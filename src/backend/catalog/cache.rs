...
impl CatalogCache {
    ...
    pub fn get_type_by_oid(&mut self, type_oid: u32) -> Option<&DataType> {
    ...
    pub fn get_type_by_name(&self, name: &str) -> Option<&DataType> {
        let oid = self.type_names.get(name)?;
        self.types.get(oid)
    }

    pub fn insert_type(&mut self, dt: DataType) {
        self.evict_if_needed();
        let oid = dt.type_oid;
