...
impl CatalogDataType {
    ...
    pub fn bool_type() -> Self {
        ...
        CatalogDataType {
            type_oid: OID_TYPE_BOOL,
            type_name: "BOOL".into(),
            type_category: TypeCategory::Boolean,
            type_length: 1,
            type_align: 1,
            is_builtin: true,
        }
    ...
    pub fn varchar(max_len: u16) -> Self {
        ...
        CatalogDataType {
            type_oid: OID_TYPE_VARCHAR,
            type_name: format!("VARCHAR({})", max_len),
            type_category: TypeCategory::String,
            type_length: -1,
            type_align: 1,
            is_builtin: true,
    ...
    pub fn date() -> Self {
        ...
        CatalogDataType {
            type_oid: OID_TYPE_DATE,
            type_name: "DATE".into(),
            type_category: TypeCategory::DateTime,
            type_length: 4,
            type_align: 4,
            is_builtin: true,
        }
 ...
 conflict    }
    pub fn bytes() -> Self {
        ...
        CatalogDataType {
            type_oid: OID_TYPE_BYTES,
            type_name: "BYTES".into(),
            type_category: TypeCategory::Binary,
            type_length: -1,
            type_align: 1,
            is_builtin: true,
        }
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        if upper.starts_with("VARCHAR") {
            ...
            return Some(CatalogDataType::varchar(max));
        }
        match upper.as_str() {
            "INT" | "INTEGER" | "INT32" => Some(CatalogDataType::int()),
            "BIGINT" | "INT64" => Some(CatalogDataType::bigint()),
            "FLOAT" | "REAL" | "FLOAT32" => Some(CatalogDataType::float()),
            "DOUBLE" | "FLOAT64" => Some(CatalogDataType::double()),
            "BOOL" | "BOOLEAN" => Some(CatalogDataType::bool_type()),
            "TEXT" | "STRING" => Some(CatalogDataType::text()),
            "DATE" => Some(CatalogDataType::date()),
            "TIMESTAMP" => Some(CatalogDataType::timestamp()),
            "BYTES" | "BYTEA" | "BLOB" => Some(CatalogDataType::bytes()),
            _ => None,
        }
    }

    /// Returns all built-in types in a canonical list
    pub fn all_builtins() -> Vec<CatalogDataType> {
        vec![
            CatalogDataType::int(),
            CatalogDataType::bigint(),
            CatalogDataType::float(),
            CatalogDataType::double(),
            CatalogDataType::bool_type(),
            CatalogDataType::text(),
            CatalogDataType::varchar(255),
            CatalogDataType::date(),
            CatalogDataType::timestamp(),
            CatalogDataType::bytes(),
        ]
    }
}
...
impl IndexType {
    ...
    pub fn from_u8(v: u8) -> Self {
        if v == 2 { Self::Hash } else { Self::BTree }
    }
}
...
impl Encoding {
    ...
    pub fn from_u8(v: u8) -> Self {
        if v == 2 { Self::ASCII } else { Self::UTF8 }
    }
}
...
impl Catalog {
    ...
    /// to `pg_oid_counter.dat` so it survives a restart.
...
impl std::fmt::Display for CatalogError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ReferencedKeyMissing => write!(
                f,
                "Referenced columns are not covered by a PK or UNIQUE constraint"
            ),
            Self::ColumnCountMismatch => {
                write!(f, "Referencing and referenced column counts differ")
            }
            Self::TypeMismatch { column } => write!(f, "Type mismatch on column '{}'", column),
            Self::ForeignKeyDependency(s) => {
                write!(f, "Cannot drop: table has foreign key dependents ({})", s)
            }
