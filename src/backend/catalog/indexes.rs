...
pub fn create_index(
    ...
    if !Path::new(&idx_file).exists() {
        let mut file = std::fs::OpenOptions::new().write(true).create(true).open(&idx_file).map_err(CatalogError::IoError)?;
...
fn allocate_index_page(file_path: &str) -> Result<u32, CatalogError> {
    let file_size = fs::metadata(file_path).map_err(CatalogError::IoError)?.len();
    let new_page_num = (file_size / crate::page::PAGE_SIZE as u64) as u32;
    let mut file = std::fs::OpenOptions::new().write(true).open(file_path).map_err(CatalogError::IoError)?;
    crate::disk::write_page(&mut file, &mut crate::page::Page::new(), new_page_num).map_err(CatalogError::IoError)?;
    Ok(new_page_num)
}
...
fn search_node(page: &crate::page::Page, key_bytes: &[u8], is_leaf: bool) -> (bool, u16) {
    ...
    while low < high {
        ...
        let key_len = if is_leaf { p_len.saturating_sub(8) } else { p_len.saturating_sub(4) };
...
pub fn index_lookup(bm: &mut BufferManager, db_name: &str, index_name: &str, key_bytes: &[u8]) -> Result<bool, CatalogError> {
    let file_path = get_index_path(db_name, index_name);
    if !Path::new(&file_path).exists() { return Ok(false); }
    let mut current_page = 0;
    loop {
        let fi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, current_page)).map_err(CatalogError::IoError)?;
        let is_leaf = bm.frames[fi].data[0] == 1;
        let next_page = if is_leaf {
            let (found, _) = search_node(&bm.frames[fi], key_bytes, true);
            bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, current_page), false).map_err(CatalogError::IoError)?;
            return Ok(found);
        } else {
...
fn insert_into_page(page: &mut crate::page::Page, key_bytes: &[u8], payload_suffix: &[u8], is_leaf: bool) -> Result<(), ()> {
    let num_keys = u16::from_le_bytes(page.data[1..3].try_into().unwrap());
    ...
    if lower + required > upper { return Err(()); }
    
    let (_, mut insert_idx) = search_node(page, key_bytes, is_leaf);
    if !is_leaf && key_bytes.is_empty() { insert_idx = 0; } 
    else if !is_leaf && insert_idx == 0 { insert_idx = 1; }
    ...
    page.data[slot_start+2..slot_start+4].copy_from_slice(&(total_payload_len as u16).to_le_bytes());
...
fn split_page(page: &mut crate::page::Page, new_page: &mut crate::page::Page, is_leaf: bool) -> Vec<u8> {
    let num_keys = u16::from_le_bytes(page.data[1..3].try_into().unwrap());
    let mid = num_keys / 2;
    
    init_leaf_page(new_page);
    if !is_leaf { new_page.data[0] = 0; }
...
pub fn insert_index_entry(bm: &mut BufferManager, db_name: &str, index_name: &str, key_bytes: &[u8], page_num: u32, slot_id: u32) -> Result<(), CatalogError> {
    let file_path = get_index_path(db_name, index_name);
    ...
    loop {
        ...
        let fi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, current_page)).map_err(CatalogError::IoError)?;
        let is_leaf = bm.frames[fi].data[0] == 1;
        let next_page = if is_leaf { None } else { Some(get_internal_child(&bm.frames[fi], key_bytes)) };
        bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, current_page), false).map_err(CatalogError::IoError)?;
        if is_leaf { break; }
    ...
    while let Some(node_num) = path.pop() {
        let fi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, node_num)).map_err(CatalogError::IoError)?;
        let is_leaf = bm.frames[fi].data[0] == 1;
        ...
        if insert_into_page(&mut bm.frames[fi], &insert_key, &insert_suffix, is_leaf).is_err() {
            ...
            let nfi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, new_page_num)).map_err(CatalogError::IoError)?;
            bm.frames[nfi].data.copy_from_slice(&new_page.data);
            bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, new_page_num), true).map_err(CatalogError::IoError)?;
            
            split = true;
            new_child = new_page_num;
        }
        
        bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, node_num), true).map_err(CatalogError::IoError)?;
    ...
    let root_fi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, 0)).map_err(CatalogError::IoError)?;
    let old_root_fi = bm.pin_page(crate::buffer_manager::PageId::new(&file_path, old_root_page_num)).map_err(CatalogError::IoError)?;
    ...
    bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, old_root_page_num), true).map_err(CatalogError::IoError)?;
    bm.unpin_page(&crate::buffer_manager::PageId::new(&file_path, 0), true).map_err(CatalogError::IoError)?;
