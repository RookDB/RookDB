...
//!   * Legacy mode – JSON-based database/global/catalog.json
//!
...
use crate::catalog::page_manager::{
    CatalogPageManager, CAT_COLUMN, CAT_CONSTRAINT, CAT_DATABASE, CAT_INDEX, CAT_TABLE, CAT_TYPE,
};
...
use crate::layout::{
    CATALOG_FILE, CATALOG_PAGES_DIR, DATABASE_DIR, GLOBAL_DIR, SYSTEM_DB_OID,
    TABLE_FILE_TEMPLATE, TABLE_DIR_TEMPLATE,
};
...
pub fn init_catalog(bm: &mut BufferManager) {
    ...
    if !base.exists() { fs::create_dir_all(base).expect("Failed to create base dir"); }

    if Path::new(CATALOG_PAGES_DIR).exists() {
        println!("Page-based catalog backend detected.");
    } else if Path::new(CATALOG_FILE).exists() {
...
pub fn load_catalog(bm: &mut BufferManager) -> Catalog {
    ...
    if Path::new(CATALOG_FILE).exists() {
        let data = fs::read_to_string(CATALOG_FILE).unwrap_or_default();
        if let Ok(cat) = serde_json::from_str::<Catalog>(&data) { return cat; }
    }
    Catalog::new()
}
...
fn load_catalog_from_pages(bm: &mut BufferManager) -> Result<Catalog, CatalogError> {
    ...
    for bytes in pm.scan_catalog(bm, CAT_DATABASE)? {
        ...
        catalog.databases.insert(name.clone(), Database {
            db_oid: oid, db_name: name, tables: HashMap::new(),
            owner, encoding: Encoding::from_u8(enc), created_at,
        });
    }
    ...
    for bytes in pm.scan_catalog(bm, CAT_TABLE)? {
        ...
        let table = Table {
            table_oid: toid, table_name: tname.clone(), db_oid,
            columns: Vec::new(), constraints: Vec::new(), indexes: Vec::new(),
            table_type: TableType::UserTable,
            statistics: TableStatistics { row_count, page_count, created_at, last_modified: 0 },
        };
        for db in catalog.databases.values_mut() {
            if db.db_oid == db_oid { db.tables.insert(tname, table); break; }
        }
    }
    ...
    for bytes in pm.scan_catalog(bm, CAT_COLUMN)? {
        ...
        let col = Column {
            column_oid: coid, name: cname, column_position: cpos,
            data_type: dt, type_modifier: tm, is_nullable,
            default_value: default_val, constraints: constraint_oids,
        };
        for db in catalog.databases.values_mut() {
            for table in db.tables.values_mut() {
                if table.table_oid == toid { table.columns.push(col.clone()); break; }
            }
        }
    }
    for db in catalog.databases.values_mut() {
    ...
    for bytes in pm.scan_catalog(bm, CAT_CONSTRAINT)? {
        ...
        for db in catalog.databases.values_mut() {
            for table in db.tables.values_mut() {
                if table.table_oid == c.table_oid { table.constraints.push(c.clone()); break; }
            }
        }
    }
        ...
        for db in catalog.databases.values_mut() {
            for table in db.tables.values_mut() {
                if table.table_oid == idx.table_oid && !table.indexes.contains(&idx.index_oid) {
                    table.indexes.push(idx.index_oid); break;
                }
            }
        }
    }
...
/// Persist in-memory Catalog to catalog.json (legacy export).
pub fn save_catalog(catalog: &Catalog) {
    let json = serde_json::to_string_pretty(catalog).expect("Failed to serialise catalog");
    let path = Path::new(CATALOG_FILE);
    if let Some(parent) = path.parent() { let _ = fs::create_dir_all(parent); }
    fs::write(path, json).expect("Failed to write catalog.json");
...
pub fn create_database(catalog: &mut Catalog, db_name: &str) -> bool {
    if db_name.is_empty() { println!("Database name cannot be empty"); return false; }
    if catalog.databases.contains_key(db_name) { println!("Database '{}' already exists", db_name); return false; }
    ...
    catalog.databases.insert(db_name.to_string(), Database {
        ...
        owner: "admin".into(), encoding: Encoding::UTF8, created_at: now_unix(),
    });
    save_catalog(catalog);

    let db_path = TABLE_DIR_TEMPLATE.replace("{database}", db_name);
    if !Path::new(&db_path).exists() { let _ = fs::create_dir_all(&db_path); }
...
pub fn create_database_enhanced(
    ...
    if db_name.is_empty() { return Err(CatalogError::InvalidOperation("Empty db name".into())); }
    if catalog.databases.contains_key(db_name) { return Err(CatalogError::DatabaseAlreadyExists(db_name.into())); }
    ...
    catalog.databases.insert(db_name.to_string(), Database {
        db_oid, db_name: db_name.to_string(), tables: HashMap::new(),
        owner: owner.to_string(), encoding, created_at,
    });
    catalog.cache.invalidate_database(db_name);
    Ok(db_oid)
}
...
pub fn drop_database(
    ...
    let table_oids: Vec<u32> = catalog.databases.get(db_name)
...
/// Legacy table creation (JSON mode).
pub fn create_table(catalog: &mut Catalog, db_name: &str, table_name: &str, columns: Vec<Column>) {
    let db_oid = match catalog.databases.get(db_name) {
        Some(db) => db.db_oid,
        None => { println!("Database '{}' does not exist.", db_name); return; }
    };
    if catalog.databases.get(db_name).unwrap().tables.contains_key(table_name) {
        println!("Table '{}' already exists.", table_name); return;
    }
    let table = Table {
        table_oid: 0, table_name: table_name.to_string(), db_oid,
        columns, constraints: Vec::new(), indexes: Vec::new(),
        table_type: TableType::UserTable, statistics: TableStatistics::default(),
    };
    catalog.databases.get_mut(db_name).unwrap().tables.insert(table_name.to_string(), table);
    catalog.cache.invalidate_table(db_oid, table_name);
    save_catalog(catalog);

    let path = TABLE_FILE_TEMPLATE.replace("{database}", db_name).replace("{table}", table_name);
    if !Path::new(&path).exists() {
        if let Ok(mut f) = OpenOptions::new().create(true).write(true).read(true).truncate(true).open(&path) {
            let _ = init_table(&mut f);
        }
    }
    println!("Table '{}' created in database '{}'.", table_name, db_name);
}
...
pub fn create_table_enhanced(
    ...
    for (pos, def) in col_defs.iter().enumerate() {
        ...
        let type_mod_val = col_defs[pos].type_modifier.map(|n| crate::catalog::types::TypeModifier::VarcharLen(n));
        ...
        columns.push(Column {
            ...
            is_nullable: def.is_nullable, default_value: def.default_value.clone(),
            constraints: Vec::new(),
        });
    }

    // Create table data file
    let file_path = TABLE_FILE_TEMPLATE.replace("{database}", db_name).replace("{table}", table_name);
    if !Path::new(&file_path).exists() {
        let mut f = OpenOptions::new().create(true).write(true).read(true).truncate(true).open(&file_path)?;
        init_table(&mut f)?;
    }
    ...
    let table = Table {
        table_oid, table_name: table_name.to_string(), db_oid, columns,
        constraints: Vec::new(), indexes: Vec::new(), table_type: TableType::UserTable,
        statistics: TableStatistics { row_count: 0, page_count: 1, created_at, last_modified: 0 },
    };
    catalog.databases.get_mut(db_name).unwrap().tables.insert(table_name.to_string(), table);
    catalog.cache.invalidate_table(db_oid, table_name);
    ...
    let (db_name, table_name) = {
        ...
        for db in catalog.databases.values() {
            for t in db.tables.values() {
                if t.table_oid == table_oid { found = Some((db.db_name.clone(), t.table_name.clone())); break; }
            }
        }
...
pub fn alter_table_add_column(
        ...
        for t in db.tables.values() {
            if t.table_oid == table_oid {
                if t.columns.iter().any(|c| c.name == col_def.name) {
                    return Err(CatalogError::InvalidOperation(format!("Column '{}' already exists", col_def.name)));
                }
    ...
    let col_oid   = catalog.alloc_oid();
    let type_mod  = col_def.type_modifier.map(|n| crate::catalog::types::TypeModifier::VarcharLen(n));
    let col_bytes = serialize_column_tuple(
        ...
        for table in db.tables.values_mut() {
            if table.table_oid == table_oid {
                table.columns.push(Column {
                    column_oid: col_oid, name: col_def.name.clone(), column_position: col_pos,
                    data_type: dt.clone(), type_modifier: type_mod.clone(), is_nullable: col_def.is_nullable,
                    default_value: col_def.default_value.clone(), constraints: Vec::new(),
                });
                break;
            }
        }
