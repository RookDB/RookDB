...
fn setup_test_table() -> (Catalog, CatalogPageManager, BufferManager, u32) {
    ...
    create_database_enhanced(
        ...
        "constraint_test_db", "admin", Encoding::UTF8,
    ...
    let table_oid = create_table_enhanced(
        &mut catalog, &mut pm, &mut bm,
        "constraint_test_db", "users", col_defs, vec![],
...
fn test_add_primary_key_constraint() {
    ...
    let result = add_primary_key_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, vec!["id".to_string()], Some("pk_users_id".to_string()),
    );
    assert!(result.is_ok(), "Should add PK constraint: {:?}", result.err());
    ...
    let pk = constraints.iter().find(|c| c.constraint_type == ConstraintType::PrimaryKey);
...
fn test_add_duplicate_primary_key_rejected() {
    ...
    add_primary_key_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, vec!["id".to_string()], None,
    ).expect("First PK should succeed");

    // Second PK should fail
    let result = add_primary_key_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, vec!["name".to_string()], None,
    );
...
fn test_add_unique_constraint() {
    ...
    let result = add_unique_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, vec!["email".to_string()], Some("uq_email".to_string()),
    );
    assert!(result.is_ok(), "Should add UNIQUE constraint: {:?}", result.err());

    // Verify constraint persisted
    let constraints = get_constraints_for_table(&catalog, &pm, &mut bm, table_oid)
        .expect("should get constraints");
    let uq = constraints.iter().find(|c| c.constraint_type == ConstraintType::Unique);
    ...
    assert!(uq_idx.is_some(), "UNIQUE constraint should create a unique non-primary index");
...
fn test_add_not_null_constraint() {
    ...
    let name_col_oid = {
        let db = catalog.databases.get("constraint_test_db").unwrap();
        let table = db.tables.get("users").unwrap();
        table.columns.iter().find(|c| c.name == "name").unwrap().column_oid
    };
    ...
    // Should be nullable initially
    {
        ...
        assert!(name_col.is_nullable, "Column should be nullable before NOT NULL constraint");
    }

    let result = add_not_null_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, name_col_oid,
    );
    assert!(result.is_ok(), "Should add NOT NULL constraint: {:?}", result.err());

    // Verify is_nullable is now false
    let db = catalog.databases.get("constraint_test_db").unwrap();
    let table = db.tables.get("users").unwrap();
    let name_col = table.columns.iter().find(|c| c.name == "name").unwrap();
    assert!(!name_col.is_nullable, "Column should be NOT NULL after constraint");
...
fn test_add_not_null_invalid_column() {
    ...
    let result = add_not_null_constraint(
        &mut catalog, &mut pm, &mut bm,
        table_oid, 99999, // non-existent column OID
    );
...
fn test_composite_primary_key() {
    ...
    let table_oid = create_table_enhanced(
        &mut catalog, &mut pm, &mut bm,
        "constraint_test_db", "enrollment", col_defs, vec![],
    ).expect("create table should succeed");
    ...
    assert!(result.is_ok(), "Composite PK should succeed: {:?}", result.err());

    // Verify constraint has 2 column OIDs
    let constraints = get_constraints_for_table(&catalog, &pm, &mut bm, table_oid)
        .expect("should get constraints");
    let pk = constraints.iter().find(|c| c.constraint_type == ConstraintType::PrimaryKey).unwrap();
    assert_eq!(pk.column_oids.len(), 2, "Composite PK should reference 2 columns");

    // Both columns should be NOT NULL
    let db = catalog.databases.get("constraint_test_db").unwrap();
    let table = db.tables.get("enrollment").unwrap();
    for col in &table.columns {
        assert!(!col.is_nullable, "Column '{}' should be NOT NULL in composite PK", col.name);
    }
...
fn test_foreign_key_column_count_mismatch() {
    ...
    assert!(result.is_err(), "Should reject FK with column count mismatch");
    match result.err().unwrap() {
